  ...
  const processOneFile = async (file: File, segmentoAtual: string, regimeEmpresa: string | null): Promise<{ servico?: any; comercio?: NotaComercio; error?: boolean }> => {
    try {
      ...
      if (isPdf) {
        ...
        console.log(`[Fiscal XML] Processando ${file.name}, tamanho: ${content.length}`);
        
        if (segmentoAtual === 'servico') {
          // 1. Tentar parser dinâmico baseado em layouts do banco
          const { parseXmlDynamic } = await import('@/utils/layoutDrivenParser');
          const dynamicResult = await parseXmlDynamic(content, file.name, regimeEmpresa);
          
          if (dynamicResult.success && dynamicResult.data) {
            console.log(`[Fiscal XML] ✅ ${file.name} parseado com layout "${dynamicResult.layout}"`);
            return { servico: dynamicResult.data };
          }
          
          // 2. Fallback: parsers estáticos legados
          console.warn(`[Fiscal XML] Layout dinâmico falhou para ${file.name}: ${dynamicResult.error}. Tentando parsers estáticos...`);
          const { parseNFSeXml, parseNFSeSpedNacional, detectAndParseXml: detect } = await import('@/utils/notaFiscalParser');
          ...
          console.error(`[Fiscal XML] ${file.name}: Nenhum parser conseguiu ler este arquivo`);
          return { error: true };
        } else {
          // Comércio - usar parser NF-e existente
