...
export function MessengerSidebar({ activeTab, onTabChange, theme, onThemeChange, viewMode, onViewModeChange }: MessengerSidebarProps) {
  ...
  const navItems: NavItem[] = [
    ...
    { id: "divider-1", label: "", icon: <></>, section: "Conversas" },
    { id: "todos", label: "Todas", icon: <MessageCircle className="w-5 h-5" /> },
    { id: "clientes", label: "Clientes", icon: <UserCircle className="w-5 h-5" /> },
    { id: "fornecedores", label: "Fornecedores", icon: <Building2 className="w-5 h-5" /> },
    { id: "interno", label: "Interno", icon: <Users className="w-5 h-5" /> },
    { id: "divider-2", label: "", icon: <></>, section: "Configurações" },
    { id: "preferencias", label: "Preferências", icon: <Settings className="w-5 h-5" /> },
  ];
            ...
            <AnimatePresence>
              {isExpanded && (
                ...
                >
                  ...
                  <p className="text-[10px] text-muted-foreground tracking-wider">NEBULA CHAT</p>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
              ...
              <AnimatePresence>
                {isExpanded && (
                  <motion.span
                    ...
                    className="text-sm font-medium whitespace-nowrap"
                  >
                    {theme === "colorful" ? "Colorido" : theme === "dark" ? "Escuro" : "Claro"}
                  </motion.span>
                )}
              </AnimatePresence>
            </motion.button>
