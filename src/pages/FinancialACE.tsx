...
import { 
  ...
  Activity, List, LayoutGrid, Target, AlertTriangle
} from "lucide-react";
...
export default function FinancialACE() {
  const [activeTab, setActiveTab] = useState<"transacoes" | "categorias" | "relatorios">("transacoes");
  const [viewMode, setViewMode] = useState<"lista" | "grid">("lista");
  const [activeFilter, setActiveFilter] = useState<FilterType>("all");
  const [modo, setModo] = useState<ModoFinanceiro>("pro"); // Will be set based on permissions
      ...
      <div className="p-4 pr-72">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex gap-2">
            ...
            >
              <Receipt className="w-4 h-4 inline mr-2" />Transações ({filteredTransacoes.length})
            </button>
            <button
